...
def graceful_shutdown(signum, frame):
    print("\n[System] Shutting down KMS server gracefully...")
    sys.exit(0)
...
from analytics.metrics_exporter import MetricsExporter
...
# ── Webapp static files ─────────────────────────────────────────────────
webapp_dir = r"d:\fyeshi\project\quantum\iptable\UI"
...
BANK_ACCOUNTS: Dict[str, dict] = {
    ...
    "HQ001": {"name": "ShorlyNot HQ", "balance": 5000000.00, "ifsc": "FQBS0000"},
}
...
async def join_session(req: JoinSessionRequest):
    ...
    try:
        result = kms.join_session(req.session_id, req.device_id, req.join_token)
        return {
...
async def deactivate_eve():
    ...
    await bus.broadcast(
        {"event": "session", "action": "eve_deactivated", "status": "GREEN"}
    )
...
async def reset_system():
    ...
    await bus.broadcast(
        {"event": "session", "action": "system_reset", "status": "GREEN", "qber": 0.0}
    )
...
def get_metrics():
    ...
    exporter = MetricsExporter(kms)
    return PlainTextResponse(exporter.prometheus_format())
...
async def get_accounts():
    return {
        "accounts": [
            {"id": k, "name": v["name"], "balance": v["balance"], "ifsc": v["ifsc"]}
...
async def transfer_funds(req: TransferRequest):
    ...
    try:
        ...
        if "compromised" in err_msg.lower() or "qber" in err_msg.lower():
            ...
            raise HTTPException(503, "Transfer blocked: quantum channel compromised")
...
@app.get("/defense", response_class=FileResponse)
async def serve_defense():
    """Serve the defense web application."""
    index_file = os.path.join(webapp_dir, "defense.html")
    if os.path.isfile(index_file):
        return FileResponse(index_file, media_type="text/html")
    return HTMLResponse("<h2>Webapp not found. Check webapp/defense.html</h2>")
...
async def startup_banner():
    ...
    print()
    print("=" * 70)
    print("  ShorlyNot — KMS v4.0 — ONLINE")
    print("=" * 70)
    print("  Bank App:  http://127.0.0.1:8000/app")
...
async def health_check():
    ...
    return {
        ...
        "security": {
            "escalation_level": kms.escalation_level,
            "active_sessions": len(kms.sessions),
            "attacks_detected": kms.attacks_detected,
        },
    }
...
async def metrics():
    ...
    health = kms.check_link_health()
    lines = [
        "# HELP shorlynot_qber Current QBER",
        "# TYPE shorlynot_qber gauge",
        f"shorlynot_qber {kms.last_qber}",
        "",
        "# HELP shorlynot_escalation_level Current escalation (0-4)",
        "# TYPE shorlynot_escalation_level gauge",
        f"shorlynot_escalation_level {kms.escalation_level}",
        "",
        "# HELP shorlynot_sessions_total Total sessions",
        "# TYPE shorlynot_sessions_total counter",
        f"shorlynot_sessions_total {kms.total_sessions}",
        "",
        "# HELP shorlynot_attacks_total Total attacks detected",
        "# TYPE shorlynot_attacks_total counter",
        f"shorlynot_attacks_total {kms.attacks_detected}",
    ]
...
@app.get("/entropy_score")
async def entropy_score():
    """Quantum entropy score endpoint."""
    from quantum_engine.quantum_entropy import quantum_entropy_score

    score = quantum_entropy_score(
        qber=kms.last_qber,
        attacks_recent=kms.attacks_detected,
        total_sessions=kms.total_sessions,
        successful_sessions=kms.total_sessions - kms.attacks_detected,
    )
    return score
...
def main():
    ...
    parser = argparse.ArgumentParser(description="ShorlyNot KMS API")
