...
class ChatGUI:
        ...
        while True:
            try:
                ...
                if text.startswith("QUANTUM_CHAT_ANNOUNCE"):
                    ...
                    self.root.after(
                        0, lambda k=new_kms, w=new_ws: self._update_urls(k, w)
                    )
            except Exception:
                pass
    ...
    def _build_ui(self):
        ...
        ttk.Entry(top, textvariable=self.user_var, width=12).grid(
            row=0, column=1, sticky=tk.W
        )
        ttk.Label(top, text="Peer").grid(row=0, column=2, sticky=tk.W, padx=4)
        ttk.Entry(top, textvariable=self.peer_var, width=12).grid(
            row=0, column=3, sticky=tk.W
        )

        ttk.Label(top, text="KMS URL").grid(row=1, column=0, sticky=tk.W, padx=4)
        ttk.Entry(top, textvariable=self.kms_var, width=34).grid(
            row=1, column=1, columnspan=3, sticky=tk.W
        )

        ttk.Label(top, text="WS URL").grid(row=2, column=0, sticky=tk.W, padx=4)
        ttk.Entry(top, textvariable=self.ws_var, width=34).grid(
            row=2, column=1, columnspan=3, sticky=tk.W
        )

        ttk.Checkbutton(
            top, text="Create new session", variable=self.new_session_var
        ).grid(row=0, column=4, sticky=tk.W, padx=8)
        ...
        ttk.Entry(top, textvariable=self.session_var, width=24).grid(
        ...
        ttk.Button(button_row, text="Connect", command=self.connect).pack(
            side=tk.LEFT, padx=4
        )
        ttk.Button(button_row, text="Disconnect", command=self.disconnect).pack(
            side=tk.LEFT, padx=4
        )
        ttk.Button(button_row, text="Link Status", command=self.fetch_status).pack(
            side=tk.LEFT, padx=4
        )
        ttk.Button(
            button_row, text="Activate Eve", command=lambda: self.control_eve(True)
        ).pack(side=tk.LEFT, padx=4)
        ttk.Button(
            button_row, text="Deactivate Eve", command=lambda: self.control_eve(False)
        ).pack(side=tk.LEFT, padx=4)
        ttk.Button(button_row, text="Trigger Attack", command=self.trigger_attack).pack(
            side=tk.LEFT, padx=4
        )
        ...
        ttk.Entry(bottom, textvariable=self.message_var).pack(
            side=tk.LEFT, fill=tk.X, expand=True, padx=4
        )
        ttk.Button(bottom, text="Send", command=self.send_message).pack(
            side=tk.LEFT, padx=4
        )
        ...
        try:
            if self.new_session_var.get():
                ...
                resp = httpx.post(
                    ...
                    json={
                        "client_a": self.user_id,
                        "client_b": self.peer_id,
    ...
    def control_eve(self, on: bool):
        threading.Thread(
            target=self._control_eve_worker, args=(on,), daemon=True
        ).start()
