...
class ChatServer:
    ...
    async def register(self, websocket, device_id: str):
        ...
        print(
            f"[ChatServer] {self.timestamp()} | OK '{device_id}' connected "
        ...
        if self._pending[device_id]:
            ...
            for msg in pending:
                try:
                    ...
                    print(
                        f"[ChatServer] {self.timestamp()} | 📨 Delivered queued "
                        f"message to '{device_id}'"
                    )
                except Exception as e:
                    print(
                        f"[ChatServer] {self.timestamp()} | ⚠️  Failed to deliver "
                        f"queued message: {e}"
                    )
    ...
    async def unregister(self, device_id: str):
        ...
        if device_id in self.clients:
            ...
            print(
                f"[ChatServer] {self.timestamp()} | ✗ '{device_id}' disconnected "
                f"({len(self.clients)} clients online)"
            )
    ...
    async def route_message(self, message: dict):
        ...
        if recipient in self.clients:
            try:
                ...
                print(
                    f"[ChatServer] {self.timestamp()} | 📨 #{self.message_count}: "
                    f"{sender} → {recipient} ({ct_bytes} bytes ciphertext)"
                )
            except Exception as e:
                print(
                    f"[ChatServer] {self.timestamp()} | ⚠️  Delivery failed to "
                    f"'{recipient}': {e}"
                )
                # Queue for later
            ...
            async for raw_message in websocket:
                ...
                if msg_type == "register":
                    ...
                    if device_id is None:
                        print(
                            f"[ChatServer] {self.timestamp()} | ⚠️  Chat message "
                    ...
                    print(
                        f"[ChatServer] {self.timestamp()} | ⚠️  Unknown message "
                        f"type: '{msg_type}'"
                    )
