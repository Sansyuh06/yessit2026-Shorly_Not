...
class NetworkGateway:
    ...
    def __init__(self, gateway_id: str = "TacNet_Gateway_01"):
        ...
        self._message_queue: Dict[
            str, deque
        ] = {}  # Recipient -> queue of pending messages
    ...
    def route_message(
        ...
        with self._lock:
            ...
            if recipient not in self._devices:
                ...
                print(
                    f"[Gateway] ⚡ Recipient '{recipient}' not online, message queued"
                )
            else:
            ...
            # Log routing
            print(
                f"[Gateway] Routed message #{self._message_counter}: {sender} → {recipient} ({size_bytes} bytes)"
            )
    ...
    def get_pending_messages(self, device_id: str) -> List[Dict[str, Any]]:
        ...
        with self._lock:
            ...
            if messages:
                print(
                    f"[Gateway] {len(messages)} message(s) delivered to '{device_id}'"
                )
