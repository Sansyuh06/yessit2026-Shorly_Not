...
def start_ui_server():
    """Start the HTML UI server."""
    subprocess.Popen(
        [sys.executable, "webapp/ui_server.py"],
        creationflags=subprocess.CREATE_NEW_CONSOLE if os.name == "nt" else 0,
    )
    print("[System] UI Server starting on http://localhost:8501")
    ...
    def _loop():
        ...
        while True:
            try:
                ...
                message = (
                    f"QUANTUM_CHAT_ANNOUNCE|{local_ip}|{kms_port}|{chat_port}".encode(
                        "utf-8"
                    )
                )
    ...
    while time.time() - start_time < 3.5:
        try:
            ...
            if text.startswith("QUANTUM_CHAT_ANNOUNCE"):
                ...
                print(
                    f"[*] Found existing Quantum Chat server at {server_ip}! Joining as client..."
                )
                return True
...
def main() -> int:
    ...
    parser.add_argument(
        "--clients", type=int, default=1, help="Number of GUI clients to open"
    )
    ...
    print(
        "[*] Checking for existing servers on the local network (takes ~3 seconds)..."
    )
