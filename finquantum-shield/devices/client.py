...
class SoldierDevice:
    ...
    def request_key(self, force_attack: bool = False) -> bool:
        ...
        try:
            self._current_key = self._kms.get_fresh_key(
                self.device_id, force_eve_attack=force_attack
            )
            print(f"[{self.device_id}] [OK] Key established")
            return True
        except Exception as e:
            print(f"[{self.device_id}] [ERROR] Key request failed: {e}")
            self._current_key = None
            return False

    def send_encrypted_message(
        self, recipient_id: str, plaintext: str
        ...
        Example:
            ...
            [Alpha_Unit] OK Encrypted message for 'Bravo_Unit'
            [Alpha_Unit]   Plaintext:  'Grid 1234'
            [Alpha_Unit]   Ciphertext: 4a8f2c1e9d3b7a...
        """
        if not self.has_key:
            print(f"[{self.device_id}] [ERROR] No key available. Call request_key() first.")
            return None
        ...
        print(f"[{self.device_id}] [OK] Encrypted message for '{recipient_id}'")
    ...
    def receive_encrypted_message(
        self, message_packet: Dict[str, Any]
    ) -> Optional[str]:
        """
        ...
        Example:
            ...
            [Bravo_Unit] OK Message from 'Alpha_Unit' verified
            [Bravo_Unit]   Plaintext: 'Grid 1234'
            'Grid 1234'
        """
        if not self.has_key:
            print(f"[{self.device_id}] [ERROR] No key available for decryption")
            return None
        ...
        try:
            ...
            print(f"[{self.device_id}] [OK] Message from '{sender}' verified")
            print(f"[{self.device_id}]   Plaintext: '{plaintext}'")

            return plaintext

        except Exception as e:
            print(f"[{self.device_id}] [ERROR] Decryption failed: {e}")
            return None
...
if __name__ == "__main__":
    ...
    for device in [soldier_a, soldier_b]:
        ...
        print(
            f"  {stats['device_id']}: Sent={stats['messages_sent']}, Received={stats['messages_received']}"
        )
