...
pytestmark = pytest.mark.skipif(
    not _server_running(), reason="KMS server not running on localhost:8000"
)
...
async def run_test():
    ...
    with httpx.Client(timeout=10) as c:
        r = c.post(
            f"{KMS}/create_session", json={"initiator": "Alpha", "peer": "Bravo"}
        )
        alpha_data = r.json()
    ...
    with httpx.Client(timeout=10) as c:
        r = c.post(
            f"{KMS}/join_session", json={"session_id": sid, "device_id": "Bravo"}
        )
        bravo_data = r.json()
    ...
    with httpx.Client(timeout=10) as c:
        fail = c.post(
            f"{KMS}/create_session", json={"initiator": "Charlie", "peer": "Delta"}
        ).json()
    p(
        "Charlie",
        f"Session attempt: {'BLOCKED' if 'error' in fail else 'unexpected success'}",
    )
