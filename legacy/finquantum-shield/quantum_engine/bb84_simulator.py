...
try:
    ...
    raise ImportError("Qiskit Aer required. Run: pip install qiskit-aer")
...
def run_bb84_session(
    ...
    if num_bits < 64:
        warnings.warn(
            "num_bits < 64. QBER estimation will be unreliable per Tomamichel et al. (arXiv:1103.4130).",
            UserWarning,
        )
    """
    ...
    if not eve:
        ...
        for i in range(num_bits):
            circuits.append(
                _build_bob_measure_circuit(alice_bits[i], alice_bases[i], bob_bases[i])
            )
        ...
        for i in range(num_bits):
            eve_circuits.append(
                _build_eve_measure_circuit(alice_bits[i], alice_bases[i], eve_bases[i])
            )
        ...
        for i in range(num_bits):
            bob_circuits.append(
                _build_eve_resend_bob_circuit(eve_bits[i], eve_bases[i], bob_bases[i])
            )
    ...
    # ── Step 5: Split into test bits (for QBER) and key bits (for encryption)
    import random
    
    num_sifted = len(sifted_alice)
    
    # ── Decoy-State Method (Hwang 2003, arXiv:quant-ph/0411006) ──────────
    decoy_mask = [secrets.randbelow(100) < int(decoy_ratio * 100) for _ in range(num_sifted)]
    
    signal_errors = sum(a != b for a, b, d in zip(sifted_alice, sifted_bob, decoy_mask) if not d)
    decoy_errors = sum(a != b for a, b, d in zip(sifted_alice, sifted_bob, decoy_mask) if d)
    ...
    # Relax threshold for small bit counts to prevent statistical variance from triggering false attacks
    pns_threshold = 0.20 if num_sifted < 1000 else 0.05
    pns_detected = abs(signal_yield - decoy_yield) > pns_threshold
    ...
    # Randomly select test bits (up to 100, or 50% if fewer sifted bits)
    num_test_bits = min(100, num_sifted // 2)
    
    # Check if we have test bits to estimate QBER
    if num_test_bits > 0:
        test_indices = random.sample(range(num_sifted), num_test_bits)
        key_indices = [i for i in range(num_sifted) if i not in test_indices]
        
        # Extract test bits (for QBER estimation)
        test_alice = [sifted_alice[i] for i in test_indices]
        test_bob = [sifted_bob[i] for i in test_indices]
        
        # Extract key bits (for encryption)
        key_alice = [sifted_alice[i] for i in key_indices]
        key_bob = [sifted_bob[i] for i in key_indices]
        
        # Step 6: Compute QBER from TEST bits only
        test_errors = sum(a != b for a, b in zip(test_alice, test_bob))
        qber = test_errors / num_test_bits
    else:
        test_indices = []
        key_indices = list(range(num_sifted))
        test_alice = []
        test_bob = []
        key_alice = sifted_alice.copy()
        key_bob = sifted_bob.copy()
        test_errors = 0
        qber = 0.0

    # Finite Key Analysis
    from quantum_engine.finite_key import hoeffding_confidence_interval, finite_key_rate, finite_key_secure_threshold
    
    qber_lower, qber_upper = hoeffding_confidence_interval(
        qber, num_test_bits, confidence=0.99
    )
    key_rate = finite_key_rate(qber, num_sifted)
    adjusted_threshold = finite_key_secure_threshold(num_test_bits, 0.11, 0.99)
    # Ensure minimum threshold to prevent normal noise from triggering false positives in small tests
    adjusted_threshold = max(0.05, adjusted_threshold)
    
    attack_detected = (qber > adjusted_threshold) or pns_detected

    # Step 7: Reconcile KEY bits
    from quantum_engine.reconciliation import cascade_reconcile
    
    key_alice, key_bob, num_corrections, corrected_indices = cascade_reconcile(
        key_alice, key_bob, num_passes=4, initial_block_size=8
    )

    raw_key = _bits_to_bytes(key_alice)
    ...
    return {
        ...
        "qubits_prepared": num_bits,
        "errors": test_errors + num_corrections,
        ...
        "qber_confidence_interval": (round(qber_lower, 6), round(qber_upper, 6)),
        "finite_key_rate": round(key_rate, 6),
...
def privacy_amplification(
    ...
    n = len(sifted_bits)
    if n < output_bits:
        raise ValueError(f"Sifted key too short for amplification: {n} < {output_bits}")
...
if __name__ == "__main__":
    ...
    print("    OK PASS — QBER below threshold")
    ...
    print("\n" + "=" * 60)
    print("  ALL BB84 CHECKS PASSED OK")
    print("=" * 60)
