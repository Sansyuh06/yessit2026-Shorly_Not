...
def establish_key(kms_url: str, device_id: str, peer_id: str) -> Optional[tuple]:
    ...
    with httpx.Client(timeout=10.0) as client:
        ...
        if "error" in data:
            ...
            print(
                f"[{device_id}]    Status: {data.get('status', '?')} | "
...
def join_existing_session(
    kms_url: str, device_id: str, session_id: str
) -> Optional[tuple]:
    ...
    try:
        async with websockets.connect(chat_url) as ws:
            ...
            sender = asyncio.create_task(
                send_loop(ws, key, device_id, peer_id, kms_url)
            )
        ...
        print("  Is chat_server.py running? Is the router allowing traffic?")
