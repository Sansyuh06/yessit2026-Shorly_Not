...
def create_bb84_protocol_diagram():
    ...
    d.add(
        Rect(
            220, 35, 130, 22, fillColor=BG_LIGHT, strokeColor=grey, strokeWidth=1, rx=3
        )
    )
    d.add(
        String(
...
def create_threat_model_diagram():
    ...
    d.add(
        String(
            225,
            108,
            "Threat Model: Quantum Channel Eavesdropping",
            fontSize=10,
            fontName="Helvetica-Bold",
            textAnchor="middle",
            fillColor=PRIMARY_DARK,
...
def add_page_elements(canvas, doc):
    ...
    canvas.drawCentredString(
        A4[0] / 2, 0.4 * inch, f"QSTCS Technical Report | Page {page_num}"
    )
...
def build_report():
    ...
    story.append(Spacer(1, 0.2 * inch))
    story.append(
        Paragraph("Technical Architecture and Security Analysis", styles["Subtitle"])
    )
    story.append(
        Paragraph("Submitted for iDEX Defence Innovation Challenge", styles["Subtitle"])
    )
    ...
    story.append(
        Paragraph(
            "Document Version 2.0 | Classification: UNCLASSIFIED | January 2026",
            styles["Footer"],
        )
    )
    ...
    for item, page in toc_items:
        story.append(
            Paragraph(f"{item} {'.' * (50 - len(item))} {page}", styles["TOCEntry"])
        )
    ...
    # THREAT LANDSCAPE
    story.append(
        Paragraph("2. Threat Landscape and Motivation", styles["SectionHeading"])
    )
    story.append(HorizontalLine(page_width, color=PRIMARY_LIGHT, thickness=1))
    ...
    story.append(
        Paragraph("2.2 Harvest Now, Decrypt Later (HNDL)", styles["SubHeading"])
    )
    story.append(
        Paragraph(
    ...
    threat_table = Table(
        threat_data, colWidths=[1.5 * inch, 1.3 * inch, 1.3 * inch, 1.3 * inch]
    )
    ...
    phases_data = [
        ...
        [
            "Transmission",
            "Encode: |0>,|1>,|+>,|-> per b[i],B[i]",
            "Choose random bases B'[i]",
            "Measured bits",
        ],
        [
            "Sifting",
            "Announce B[i] over classical channel",
            "Compare B'[i], keep matches",
            "Sifted key (~50%)",
        ],
        [
            "Verification",
            "Sample subset, compute QBER",
            "QBER < 11%: Accept",
            "256-bit raw key",
        ],
    ]
    phases_table = Table(
        phases_data, colWidths=[0.8 * inch, 1.9 * inch, 1.8 * inch, 1.1 * inch]
    )
    ...
    workflow_data = [
        ...
        [
            "1",
            "Device A authenticates to KMS, requests session key",
            "Device identity verified",
        ],
        [
            "2",
            "KMS executes BB84 simulation (512 qubits)",
            "Quantum randomness generated",
        ],
        ["3", "Sifting produces ~256 correlated bits", "Basis reconciliation complete"],
        [
            "4",
            "KMS computes QBER; verifies < 11% threshold",
        ...
        [
            "10",
            "Device B decrypts and verifies auth tag",
            "Message authenticity confirmed",
        ],
    ]
    workflow_table = Table(
        workflow_data, colWidths=[0.5 * inch, 3.2 * inch, 1.9 * inch]
    )
...
|-- tests/                             # Automated security verification tests"""
    ...
    # ============================================================
    story.append(
        Paragraph("8. Conclusion and Development Roadmap", styles["SectionHeading"])
    )
    story.append(HorizontalLine(page_width, color=PRIMARY_LIGHT, thickness=1))
    ...
    roadmap_data = [
        ...
        [
            "Near-term",
            "Integration with commercial QKD hardware (QNu Labs)",
            "6-12 months",
        ],
