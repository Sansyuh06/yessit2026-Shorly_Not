...
for ShorlyNot — a quantum-aware banking security platform.
"""
...
class KeyManagementService:
    ...
    def _derive_aes_key(
        self, raw_key: bytes, pqc_secret: Optional[bytes], hkdf_salt: bytes
    ) -> bytes:
    ...
    def _update_escalation(self, status: str) -> None:
        ...
        if self.escalation_level == 1:
            ...
            # Note: Clears burned_ports to recycle them for future L1 cycles if needed.
    ...
    def create_session(
        ...
        if use_hybrid:
            ...
            qsh = HybridHandshake()
            pqc_secret = qsh.perform_handshake(client_a, client_b)
        hkdf_salt = os.urandom(16)
        aes_key = self._derive_aes_key(amplified_key, pqc_secret, hkdf_salt)
        ...
        if status == "RED":
            raise Exception(
                f"Quantum channel compromised — QBER={qber:.2%}. Session aborted."
            )

        join_token = hmac.new(aes_key, session_id.encode(), hashlib.sha256).hexdigest()[
            :16
        ]
    ...
    def join_session(
        self, session_id: str, device_id: str, join_token: str = None
    ) -> Dict[str, object]:
        with self._lock:
        ...
        if join_token is None or not hmac.compare_digest(record.join_token, join_token):
            raise PermissionError("Invalid or missing join_token")
    ...
    def get_fresh_key(
        ...
        """
        with self._lock:
            current_eve_mode = self.eve_mode

        if force_eve_attack:
            current_eve_mode = True

        session_id = uuid.uuid4().hex
        bb84 = run_bb84_session(session_id, num_bits=768, eve=current_eve_mode)
        ...
        if pqc_enabled:
            ...
            qsh = HybridHandshake()
            pqc_secret = qsh.perform_handshake(device_id, peer_id)
        hkdf_salt = os.urandom(16)
        aes_key = self._derive_aes_key(raw_key, pqc_secret, hkdf_salt)
    ...
    def trigger_attack(self) -> Dict[str, object]:
        ...
        aes_key = self._derive_aes_key(os.urandom(32), None, os.urandom(16))
        record = SessionRecord(
            session_id=session_id,
            key=aes_key,
            qber=qber,
    ...
    def check_link_health(self) -> Dict[str, object]:
        with self._lock:
            return {
                ...
                "attacks_detected": self.attacks_detected,
                "active_sessions": sum(
                    1 for s in self.sessions.values() if not s.is_control
                ),
