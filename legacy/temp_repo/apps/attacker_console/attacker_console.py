...
def safe_api_call(method: str, url: str, **kwargs):
    ...
    try:
        r = getattr(httpx, method.lower())(url, timeout=kwargs.pop("timeout", 10), **kwargs)
        r.raise_for_status()
        ...
        console.print(f"[bold red]HTTP {e.response.status_code}: {e.response.text}[/bold red]")
        return None
...
def attack_single(kms_url: str):
    ...
    if not safe_post(f"{kms_url}/activate_eve", timeout=5): return
...
def reset(kms_url: str):
    if safe_post(f"{kms_url}/deactivate_eve", timeout=5) is None: return
    if safe_post(f"{kms_url}/reset", timeout=5) is None: return
